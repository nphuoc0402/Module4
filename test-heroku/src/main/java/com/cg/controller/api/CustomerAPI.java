...
public class CustomerAPI {
    ...
    public ResponseEntity<?> showUpdateForm(@PathVariable Long id) {
        ...
        if (customer.isPresent()) {
            Optional<LocationRegion> locationRegion = locationRegionService.findById(customer.get().getLocationRegion().getId());
            LocationRegionDTO locationRegionDTO = locationRegion.get().toLocationRegionDTO();

            Map<String, Object> result = new HashMap<>();
            result.put("customer", customer.get());
    ...
    public ResponseEntity<?> doDeposit(@Validated @RequestBody DepositDTO depositDTO, BindingResult bindingResult) {
        ...
        try {
            ...
            CustomerDTO customerDTO = customerService.findById(depositDTO.getCustomerId()).get().toCustomerDTO();
        ...
        if (customer.isPresent()) {
            ...
            if (current_balance.compareTo(transactionAmount) != -1) {
                try {
                    ...
                    CustomerDTO customerDTO = customerService.findById(withdrawDTO.getCustomerId()).get().toCustomerDTO();
